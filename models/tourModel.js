...
const tourSchema = new mongoose.Schema({
    ...
    difficulty: { type: String, required: [true, "A tour must have a difficulty"], enum: { values: ["easy", "medium", "difficult"], message: "Difficulty is either: easy, medium or difficult!" } },
    ratingsAverage: { type: Number, default: 4.5, min: [1, "Rating must be above 1.0"], max: [5, "Rating must be below 5"], set: val => Math.round(val * 10) / 10 },
...
tourSchema.pre("aggregate", function(next) {
    const elements = this.pipeline()[0];
    if (Object.keys(elements)[0] !== "$geoNear") {
